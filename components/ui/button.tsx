...
const buttonVariants = cva(
  cn(
    'group shrink-0 flex-row items-center justify-center gap-2 rounded-lg shadow-lg',
    Platform.select({
...
function Button({ className, variant, size, text, ...props }: ButtonProps) {
  return (
    <Pressable
      className={cn(props.disabled && 'opacity-50', buttonVariants({ variant, size }), className)}
      role="button"
      {...props}>
      <Text className={buttonTextVariants({ variant, size })}>{text}</Text>
    </Pressable>
  );
}
